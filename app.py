...
def isLogined():
    if twitter_session is not None:
        return False
    else:
        return True
...
def index():
    ...
    if request.method == 'POST':
        ...
        validate_result = ValidateUserIds(user_id_1, user_id_2, twitter_session)
        if validate_result is not None:
            flash (validate_result, 'alert alert-danger')
            return render_template('index.html', form = form)
        else:
            message1, message2 = check_result(user_id_1, user_id_2, twitter_session)
